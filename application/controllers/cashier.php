...
class Cashier extends Secure_area
{
	...
	{
		parent::__construct('cashier','items');
		...
		else
		{
			...
			else
			{
				if ($this->sale_lib->get_email_receipt() && !empty($cust_info->email))
				{
					...
					$this->email->message($this->load->view("cashier/receipt_email",$data, true));	
					$this->email->send();
				}
			}
			$this->load->view("cashier/receipt",$data);
			$this->sale_lib->clear_all();
		}
	...
	function _reload($data=array())
	{
		...
		$this->load->view('cashier/register',$data);
		$this->_remove_duplicate_cookies();
	...
	function suspended()
	{
		...
		$this->load->view('cashier/suspended', $data);
	}
	...
	function unsusp()
	{
		$trans_no = $this->input->post('trans_no');
		$this->sale_lib->clear_all();
		$this->sale_lib->copy_entire_suspended_sale($trans_no);
		$this->Sale_suspended->delete($trans_no);
    	$this->_reload();
	}
}
?>